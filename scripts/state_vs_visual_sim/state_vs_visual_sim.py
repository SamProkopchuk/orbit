...
num_envs_lst = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192]
seeds = [0, 1, 2]

# TODO
# visual-n2048-s0 takes >10 hours without ever starting the training loop
# find a way to detect this and kill the process

# TODO get reliable gpu memory usage
...
for seed in seeds:
    for num_envs in num_envs_lst:
        for camera in ["state", "visual"]:
            ...
            command = f"./orbit.sh -p source/standalone/workflows/skrl/train.py --task {task} --headless --num_envs {num_envs} --seed {seed} --timesteps 1000 --usewandb"
            
            # instead of using wandb here, we run wandb inside the process, so we can log system metrics
